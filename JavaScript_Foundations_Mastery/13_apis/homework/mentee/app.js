...
console.log("Total number of users: " + user.length);
console.log("Name of first user: " + user[0].name);
console.log("Email of last user: " + user[user.length - 1].email);
console.log("City of third user: " + user[2].address.city);
console.log("Comapny name of user with id 7: " + user[6].company.name);
...
const filterEmails = user.filter(user =>{
  if(user.email.endsWith(".biz")){
    return {email: user.email, name: user.name};
  }
  return null;
})
console.log("Users email ends with '.biz':");
filterEmails.forEach(element => {
  console.log("Email: " + element.email + ", Name: " + element.name);
});

const userSamantha = user.find(user =>{
  if(user.username === "Samantha"){
    return user;
  }
});
if(userSamantha){
  console.log(userSamantha.address.street + ", " + userSamantha.address.city);
} else {
  console.log("no user found");
}

user.forEach(element => {
  console.log(`Name: ${element.name} | Company: ${element.company.name}`);
});
...
// Answer: https://jsonplaceholder.typicode.com/posts?_page=5&_limit=4 , I will just
// change _page=3 to _page=5
...
const weatherJson = `{"latitude":40.710335,"longitude":-73.99308,
"generationtime_ms":0.14901161193847656,"utc_offset_seconds":0,"timezone":"GMT",
"timezone_abbreviation":"GMT","elevation":27.0,"current_weather_units":
{"time":"iso8601","interval":"seconds","temperature":"°C","windspeed":"km/h",
"winddirection":"°","is_day":"","weathercode":"wmo code"},"current_weather":
{"time":"2026-08-03T03:30","interval":900,"temperature":24.4,"windspeed":14.0,
"winddirection":125,"is_day":0,"weathercode":3}}`; // ← replace {} with the real response
...
console.log("Current temperature: " + weather.current_weather.temperature + weather.current_weather_units.temperature);
console.log("Wind speed: " + weather.current_weather.windspeed + " " + weather.current_weather_units.windspeed);

console.log("Weather code: " + weather.current_weather.weathercode + " " + weather.current_weather_units.weathercode);
...
console.log("----------Task 6----------");
const nycWeatherJson = `{"latitude":40.710335,"longitude":-73.99308,
"generationtime_ms":0.13077259063720703,"utc_offset_seconds":0,"timezone":"GMT",
"timezone_abbreviation":"GMT","elevation":27.0,
"current_weather_units":{"time":"iso8601","interval":"seconds","temperature":"°C",
"windspeed":"km/h","winddirection":"°","is_day":"","weathercode":"wmo code"},
"current_weather":{"time":"2026-08-04T02:45","interval":900,"temperature":20.6,
"windspeed":12.2,"winddirection":317,"is_day":0,"weathercode":0}}`; // ← NYC response

const londonWeatherJson = `{"latitude":51.51147,"longitude":-0.13078308,
"generationtime_ms":0.20241737365722656,"utc_offset_seconds":0,"timezone":"GMT",
"timezone_abbreviation":"GMT","elevation":29.0,
"current_weather_units":{"time":"iso8601","interval":"seconds","temperature":"°C",
"windspeed":"km/h","winddirection":"°","is_day":"","weathercode":"wmo code"},
"current_weather":{"time":"2026-08-04T02:45","interval":900,"temperature":21.2,
"windspeed":3.2,"winddirection":80,"is_day":0,"weathercode":2}}`; // ← London response
...
function analyseUser(user, posts) {
  ...
  posts.forEach(post => {
    ...
    } else if (shortestTitle.length > post.title.length){
      shortestTitle = post.title;
    }
  });
...
function parseUrl(url) {
  ...
  const baseAndParam = url.split("?");
  const base = baseAndParam[0];
  let queryString = baseAndParam[1];
